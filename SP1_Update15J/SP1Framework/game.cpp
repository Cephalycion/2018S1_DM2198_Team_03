...
int monsterRenderOrNot = 0; // to only render monsters once

vector<int> sequence; // vector for sequence for minigame
int redTrue, blueTrue, yellowTrue, greenTrue = 0; // for showing the values of the vector in the interface
...
void titleLogic()
{
	...
	if ((g_abKeyPressed[K_DOWN]) && (titleChoice < 2)) // 1 to 2
	{
		titleChoice++;
...
void checkSequence()
{
	...
	if (bSomethingHappened)
	{
		g_dScanTime = g_dElapsedTime + 5.00;
	}
}
...
void scanForDisplay()
{
	...
	if (bSomethingHappened)
	{
		g_dScanTime = g_dElapsedTime + 1.00; //put 3.50
	}
}
...
void checkInput()
{
	...
	if (bSomethingHappened)
	{
		g_dSequenceTime = g_dElapsedTime + 0.125;
	}
}
